...
--local _tCSVTableByType              = {[CSV_TYPE_BASE] =_tCSVBase, [CSV_TYPE_FINAL] = _tCSVFinal};
local _tRowTypes                    = {[CSV_TYPE_BASE] = "BaseRow", [CSV_TYPE_FINAL] = "FinalRow"}; --used for type in GetRow QUESTION do i need this???????
...
GetRow = function(sGrid, nRow)
    ...
    if _nColumnCount > 0 then
        ...
        for nColumn = 1, _nColumnCount do
            local sCellText             = GetCellText(sGrid, nRow, nColumn);
            local sColumn               = _tHeaders[nColumn]:upper();
            tByColumnID[nColumn]        = sCellText;
            tByColumnName[sColumn]      = sCellText;
    ...
    local tRetMeta = {
        ...
        __index = function(t, k)
            ...
            --try to get the column name and number
            if (zK == "number" and k <= _nColumnCount) then
                --sRet = tByColumnID[k] or nil;
                nColumn = k;
                sColumn = _tHeaders[nColumn];
            elseif (zK == "string") then
...
--[[!
@fqxn CFS.Classes.ProcSys.CSV.Methods.LoadToGrid
...
--Log.Note(UserEnv.Get()._tRow.Name)
        ...
        local vProcRet = _fRowProc(nRow, nColumn, sHeader, GetRow(_sBaseDataGrid, nRow), sText, GetFinalValue);
...
UpdateActiveCell = function(nRow, nColumn)
    _nCurrentRow    = nRow;

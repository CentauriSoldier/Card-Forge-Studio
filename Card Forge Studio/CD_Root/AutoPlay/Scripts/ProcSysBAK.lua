...
local _tCSV                         = null; --the entire CSV file content
local _tCSVBase                     = null; --the filters CSV content
local _tCSVFinal                    = null; --the processed, filtered CSV content
local _tCSVHeaders                  = null;
local _tCSVRowFilterMap             = {};
------------------------------------- Exporters
...
--[[!
@fqxn CFS.Classes.ProcSys.Methods.CSVFilter
@desc Filters the <em>_tCSV</em> table rows using the active RowFilter, resets the appropriate variables, and loads the selected rows into the <em>_tCSVBase</em> table.
@note This function does change the contents of the <em>_tCSV</em> table.
...
--[[!
@fqxn CFS.Classes.ProcSys.Methods.CSVLoadFromFile
@desc Loads the specified CSV file into the _tCSV and _tCSVHeaders table.
@param string pFile Full path to the CSV file to load.
@note --Assumes file path if good (for now). May change later
@vis Static Private
!]]
CSVLoadFromFile = function(pFile)
    --load the CSV file
    local _tCSV, _tCSVHeaders = FTCSV.parse(pFile);
...
--[[!
@fqxn CFS.Classes.ProcSys.Methods.CSVLoadToGrid
...
!]] --TODO SPLIT THIS FUNCTION into two...UpdateGrid
CSVLoadToGrid = function(pFile)
    _bIsLoading     = true;
    PrepUpdateGrids();
    ...
    for nRow, tRow in ipairs(_tBaseCSV) do
